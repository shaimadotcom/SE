...
class _QuestionViewState extends State<QuestionView> {
  ...
  void initState() {
    ...
    //questions?.forEach((element) {logger.i(element.toJson()); });
  }
// int selected=-1;
//   int selected1=-1;
//   int selected2=-1;
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    if (groupvalue![currentQuestionIndex! % 4] != -1) {
                      if (currentQuestionIndex! % 4 < questions!.length - 1) {
                           ...
                           logger.i(currentQuestionIndex);
                          logger.i(questions![currentQuestionIndex! % 4].correctAnswer);
                  ...
                  child: currentQuestionIndex! % 4 == questions!.length - 1
                      ? Provider.of<ProfileProvider>(context, listen: true).isLoading
                          ...
                          : Text(getTranslated("Submit", context) ?? "Submit")
                      : Text(getTranslated("Next", context) ?? "Next"),
                ),
              ],
            )),
      ],
  ...
  Future<void> route(bool done, String message) async {
    if (done) {
      ...
      await Provider.of<SplashProvider>(context, listen: false).initConfig(context).then((bool isSuccess) {
        if (isSuccess) {
          if (int.parse(Provider.of<SplashProvider>(context, listen: false).configModel!.data!.user!.total_points!.toString()) >= int.parse(Provider.of<SplashProvider>(context, listen: false).configModel!.data!.pointsToOnline!.toString()) && Provider.of<SplashProvider>(context, listen: false).showIntro()!) {
