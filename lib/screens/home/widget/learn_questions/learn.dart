  ...
  Widget StepsWidget(BuildContext context) {
    return Consumer<SplashProvider>(builder: (context, splashProvider, child) {
      return GridView.builder(
          ...
          itemCount: splashProvider.configModel!.data!.offlineLevels!.length,
          padding: const EdgeInsets.all(0),
          primary: false,
          shrinkWrap: true,
          itemBuilder: (context, index) {
            return gridItem(splashProvider.configModel!.data!.offlineLevels![index]);
          });
    });
  }

  void _handleClick(OfflineLevels level) {
    Navigator.of(context).push(MaterialPageRoute(
        builder: (BuildContext context) => OfflineLevelsPage(
              level: level,
            )));
  }

  Widget gridItem(OfflineLevels level) {
